  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='w-full'>
        ...
        <input onChange={(e) => setName(e.target.value)} value={name} className='w-full max-w-[500px] px-3 py-2' type="text" name="" id="" placeholder='Nhập tên sản phẩm' required/>
      </div>
      <div className='w-full'>
        <p className='mb-2'>Mô tả</p>
        <textarea onChange={(e) => setDescription(e.target.value)} value={description} className='w-full max-w-[500px] px-3 py-2' type="text" name="" id="" placeholder='Nhập mô tả' required/>
      </div>
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        ...
        <div>
          <p className='mb-2'>Kiểu sản phẩm</p>
          <select onChange={(e) => setSubCategory(e.target.value)} className='w-full px-3 py-2'>
            <option value="Topwear">Áo</option>
            <option value="Bottomwear">Quần</option>
            <option value="Ưinterwear">Quần áo đông</option>
          </select>
        </div>
        <div>
          <p className='mb-2'>Giá sản phẩm</p>
          <input onChange={(e) => setPrice(e.target.value)} value={price} type="Number" placeholder='0' className='w-full px-3 py-2 sm:w-[120px]'/>
        </div>
      </div>
      <div>
      ...
      <div className='flex gap-2 mt-2'>
        <input onChange={() => setBestseller(prev => !prev)} checked={bestseller} type="checkbox" id='bestseller' className='w-4'/>
